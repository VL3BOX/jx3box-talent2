...
(async () => {
    ...
    const getSkill = async (id) => {
        ...
        if (length > 1) {
            ...
            desc = await Promise.all(desc.map(d1 => filter(d1, datas)));
            desc.map(d2 => d2 && d2.replace(/"/g, "'"))
        } else {
            desc = res[res.length - 1]["Desc"];
            desc = await filter(desc, datas); //过滤后的奇穴描述
    ...
    for (let k in datas.raw) {
        // 固定6行，每行4个
        datas.output[datas.talent_relation[k]] = [[], [], [], [], [], []];
        let list = datas.raw[k];
        // 每一行
        for (let r = 0; r < list.length; r++) {
            let row = list[r];
            for (let i = 0; i < row.length; i++) {
                let item = row[i];
                let res = await getSkill(item);
                datas.output[datas.talent_relation[k]][r][i] = res;
                bar.tick(1, { doing: `${k} | ${res?.name}(${res?.id})`, title: bar.title });
            }
        }
    }
    bar.terminate();
